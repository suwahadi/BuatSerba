    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 sm:p-6">
        ...
        <form wire:submit.prevent="confirmSubmit" class="space-y-4 sm:space-y-6">
            ...
            <div>
                ...
                <select
                    id="selectedOrderId"
                    wire:model.live="selectedOrderId"
            ...
            <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                    Bukti Foto (Opsional, maksimal 3 foto)
                </label>
                        ...
                        @foreach($uploadedPhotos as $index => $photo)
                            <div class="relative group">
                                ...
                                <button 
                                    ...
                                    class="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs shadow-md transition"
                                    @if($showConfirmDialog) disabled @endif
                                >
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if(count($uploadedPhotos) < 3)
                    <div class="relative">
                        ...
                        <label 
                            for="photo-upload"
                            class="flex items-center justify-center gap-2 w-full px-4 py-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-green-500 hover:bg-green-50 transition {{ $showConfirmDialog ? 'opacity-50 cursor-not-allowed' : '' }}"
                        >
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                            </svg>
                            <span class="text-sm text-gray-600">
                                Klik untuk upload foto ({{ 3 - count($uploadedPhotos) }} slot tersisa)
                            </span>
                        </label>
                        
                        <!-- Loading indicator for upload -->
                        <div wire:loading wire:target="photos" class="absolute inset-0 bg-white/80 flex items-center justify-center rounded-lg">
                ...
                @error('photos.*')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">Format: JPG, PNG, WEBP. Maksimal 5MB per foto.</p>
            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            @if(count($uploadedPhotos) > 0)
                                <div class="flex justify-between items-start">
                                    ...
                                    <span class="font-medium text-gray-900">{{ count($uploadedPhotos) }} foto</span>
                                </div>
                            @endif
                            @if($note)
                                <div class="flex justify-between items-start">
