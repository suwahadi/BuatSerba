        ...
        <div class="grid grid-cols-1 {{ $cartItems->count() > 0 ? 'lg:grid-cols-3' : '' }} gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow-lg p-4 sm:p-6 sticky top-20 sm:top-24">
                    ...
                    <!-- Action Buttons -->
                    @if($cartItems->count() > 0)
                    <div class="space-y-2 sm:space-y-3" x-data="{ isLoading: false }" 
                         x-init="
                            // Clear any leftover sessionStorage when on cart page (not checkout)
                            if (!window.location.pathname.includes('/checkout')) {
                                sessionStorage.removeItem('checkoutLoading');
                            }
                            
                            // Listen for checkout page ready event (in case user navigates back)
                        ...
                        <button wire:click="checkout" 
                                ...
                                class="w-full bg-green-600 text-white py-2.5 sm:py-3 rounded-lg text-sm sm:text-base font-semibold hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                            <span wire:loading.remove wire:target="checkout">Lanjut Checkout</span>
